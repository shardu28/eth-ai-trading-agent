...
class DeltaExchangeAPI:
    ...
    def __init__(self, config_path: str = "config.yml"):
        ...
        # Read from environment variables (GitHub Secrets) if available
        # Otherwise use config file values
        import os
        self.api_key = os.getenv('DELTA_API_KEY') or self.config['api']['api_key']
        self.api_secret = os.getenv('DELTA_API_SECRET') or self.config['api']['api_secret']
        
        # Validate credentials
        if not self.api_key or not self.api_secret:
            raise ValueError(
                "API credentials not found!\n"
                "For GitHub: Set DELTA_API_KEY and DELTA_API_SECRET as repository secrets\n"
                "For local: Fill api_key and api_secret in config.yml"
            )
        ...
        logger.info(f"Credentials source: {'Environment variables' if os.getenv('DELTA_API_KEY') else 'Config file'}")
    ...
    def _make_request(self, method: str, endpoint: str, params: Dict = None, 
                     data: Dict = None, weight: int = 1, authenticated: bool = True) -> Tuple[bool, Any]:
        """
        Make HTTP request to Delta Exchange
        
        Args:
            authenticated: If False, skips signature generation (for public endpoints)
        ...
        if authenticated:
            ...
            # Public endpoints - minimal headers
            headers = {
                'User-Agent': 'python-rest-client',
                'Content-Type': 'application/json'
            }
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                try:
                    ...
                    logger.error(f"Authentication failed. Raw response: {response.text}")
    ...
    def get_product_by_symbol(self, symbol: str) -> Tuple[bool, Any]:
        ...
        """
        return self._make_request('GET', f'/v2/products/{symbol}', weight=3, authenticated=False)
